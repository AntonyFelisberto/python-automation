...
image_objects = [cv2.imread(f"collage/{filename}") for filename in images]
print(images)

shape = cv2.imread(f"collage/{images[0]}").shape
print(shape)

big_image = np.zeros((
                        shape[0] * rows + horizontal_margin * (rows + 1), 
...
cv2.imwrite("grid.png", big_image)
